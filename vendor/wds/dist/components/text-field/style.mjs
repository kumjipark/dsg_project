...
const textFieldWrapperStyle = ({ invalid, readOnly, type, disabled, width = "initial", height = "auto", xs, sm, md, lg, xl }) => (theme) => css`
    display: flex;
    align-items: center;
    border-radius: 12px;
    border: none;
    ...
    ${EXCLUDE_TYPE.includes(type || "") && css`
      input {
        max-height: 24px;
      }
...
`;
const positiveIconWrapperStyle = (theme) => css`
  position: relative;
...
`;
const textFieldContentStyle = css`
  flex-shrink: 0;
  width: fit-content;
  height: fit-content;
  max-height: 24px;
`;
const textFieldButtonStyle = ({ variant, disabled }) => (theme) => css`
    box-shadow: none;
    height: 100%;
    align-items: center;
    padding: 12px 16px;
