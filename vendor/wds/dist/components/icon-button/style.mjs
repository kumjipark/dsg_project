...
const iconButtonStyle = ({ xs, sm, md, lg, xl, ...props }) => (theme) => css`
...
const iconButtonSizeStyle = (params) => {
	...
	if (typeof size === "number") return css`
      font-size: ${size}px;
      width: fit-content;
      height: fit-content;
      ...
      ${variant === "background" && css`
        padding: 2px;
        font-size: ${size - 4}px;
      `}

      ${(variant === "solid" || variant === "outlined") && css`
        padding: 6px;
        width: ${size}px;
        height: ${size}px;
...
const iconButtonColorStyle = ({ variant, color, interactionColor, alternative }, theme) => {
	switch (variant) {
      ...
      `;
		case "background": return css`
        border: none;
        box-shadow: none;
        background-color: transparent;
        color: ${alternative ? addOpacity(theme.semantic.static.white, theme.opacity[88]) : addOpacity(theme.semantic.static.black, theme.opacity[43])};

        ${!alternative && css`
          @supports (-webkit-backdrop-filter: none) {
            color: ${addOpacity(theme.atomic.coolNeutral[50], theme.opacity[61])};
          }
        `}
        ...
        &::before {
          ...
          ${alternative ? css`
                background-color: ${addOpacity(theme.atomic.coolNeutral[30], theme.opacity[61])};
              ` : css`
                background-color: ${addOpacity(theme.semantic.static.white, theme.opacity[52])};
...
const backgroundBlendStyle = (theme) => css`
  position: absolute;
  content: '';
  background-color: ${addOpacity(theme.semantic.static.black, theme.opacity[5])};
  width: auto;
  height: calc(100% + 8px);
  aspect-ratio: 1 / 1;
  top: 50%;
  ...
  @supports (-webkit-backdrop-filter: none) {
    background-color: ${addOpacity(theme.semantic.static.black, .14)};
  }
`;
//#endregion
