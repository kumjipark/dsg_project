'use client';
import { MEDIUM_SCALE_RATIO, SMALL_SCALE_RATIO } from "./constants.mjs";
...
const getPaginationDotScale = ({ index, visibleArea, maxDotCount, totalPages }) => {
	...
	if (visibleArea[0] === 1 && index === 1 || visibleArea[1] === totalPages - 2 && index === totalPages - 2) return MEDIUM_SCALE_RATIO;
	if (distance === 1) return maxDotCount <= 4 ? 1 : MEDIUM_SCALE_RATIO;
	if (distance === 0) return maxDotCount <= 4 ? MEDIUM_SCALE_RATIO : SMALL_SCALE_RATIO;
	return 1;
};
//#endregion
