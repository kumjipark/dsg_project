...
const cardListStyle = ({ xs, sm, md, lg, xl, width, platform }) => (theme) => css`
    width: ${width ?? "100%"};
    max-width: 100%;
	...
	xl
      ...
      `)}
  `;
const cardListContentStyle = css`
  width: 24px;
  height: 24px;
  font-size: 24px;
`;
...
const cardListSkeletonStyle = ({ xs, sm, md, lg, xl, width, platform, hasLeadingContent, hasTrailingContent }) => (theme) => css`
    width: ${width ?? "100%"};
    max-width: 100%;
