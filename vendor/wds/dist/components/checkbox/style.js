...
//#region src/components/checkbox/style.ts
const checkboxStyle = ({ size, bold, checked, indeterminate, disabled, tight, xs, sm, md, lg, xl }) => (theme) => _wanteddev_wds_engine.css`
    display: flex;
    align-items: center;
    ...
    ${(checked || indeterminate) && _wanteddev_wds_engine.css`
      [data-role='checkbox-icon-wrapper'] {
  ...
  ${disabled && _wanteddev_wds_engine.css`
	...
	tight
      ...
      `)}
  `;
const checkboxSizeStyle = ({ size, bold, tight }) => {
	switch (size) {
        ...
        ${tight === true && _wanteddev_wds_engine.css`
          padding-left: 1px;
          padding-right: 1px;
          width: 20px;
