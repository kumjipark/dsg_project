import { SerializedStyles as SerializedStyles$1 } from "../../node_modules/.pnpm/@emotion_utils@1.4.2/node_modules/@emotion/utils/dist/declarations/src/types.js";
import { Theme } from "@wanteddev/wds-engine";

//#region src/components/bottom-navigation/style.d.ts
declare const bottomNavigationStyle: (theme: Theme) => SerializedStyles$1;
declare const bottomNavigationItemStyle: (theme: Theme) => SerializedStyles$1;
//#endregion
export { bottomNavigationItemStyle, bottomNavigationStyle };