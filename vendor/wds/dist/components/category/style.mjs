...
const categoryListStyle = ({ isScrollableLeft, isScrollableRight, horizontalPadding, verticalPadding, size, xs, sm, md, lg, xl }) => (theme) => css`
    width: 100%;
    list-style: none;
    position: relative;
    padding: 0;
	...
	isScrollableRight
        ...
        ${(params?.horizontalPadding !== void 0 || params?.size !== void 0) && css`
          ${categorySizeStyle({
...
const categorySizeStyle = ({ size, verticalPadding }) => {
	switch (size) {
        ...
        [data-role='category-list-wrapper'] {
          ...
          ${verticalPadding === true && css`
            padding: 8px 0px;
          `}

          ${verticalPadding === false && css`
            padding: 0px;
          `}
        }
      `;
...
`;
const categoryListItemStyle = ({ size, xs, sm, md, lg, xl }) => (theme) => css`
