...
`;
const modalContainerWrapperStyle = ({ variant, xs, sm, md, lg, xl }) => (theme) => css`
    position: fixed;
    display: flex;
...
`;
const modalContainerStyle = ({ resize, variant, size, xs, sm, md, lg, xl }) => (theme) => css`
    display: flex;
	...
	xl
        ...
        ${(params?.resize || params?.size || params?.variant) && css`
          ${modalContainerSize(getPreviousValue({
	xs,
	sm,
	md,
	lg,
	xl
}, "size", size, breakpoint), getPreviousValue({
	xs,
	sm,
	md,
	lg,
	xl
          ...
          ${modalContainerVariant(getPreviousValue({
	xs,
	sm,
	md,
	lg,
...
const modalGrabberStyle = (theme) => css`
  min-width: inherit;
  position: absolute;
...
`;
const modalContentStyle = ({ gap, xs, sm, md, lg, xl }) => (theme) => css`
    width: 100%;
