...
const require_components_date_picker_constants = require("./constants.js");
...
const getRegexFormat = (format, locale) => {
	switch (format) {
		...
		case "M": return /\d{1,2}/;
		case "MM": return /^\d{2}/;
		case "MMM": return new RegExp("^" + String.raw`${require_components_date_calendar_helpers.getMonths(locale).map((v) => v.short).join("|")}`);
		case "MMMM": return new RegExp("^" + String.raw`${require_components_date_calendar_helpers.getMonths(locale).map((v) => v.long).join("|")}`);
		case "D": return /^\d{1,2}/;
		case "DD": return /^\d{2}/;
		case "H": return /^\d{1,2}/;
		case "HH": return /^\d{2}/;
		...
		case "s": return /^\d{1,2}/;
		case "ss": return /^\d{2}/;
		case "a": return new RegExp("^" + String.raw`${require_components_date_calendar_helpers.getMeridiem(locale).map((v) => v.lower).join("|")}`);
		case "A": return new RegExp("^" + String.raw`${require_components_date_calendar_helpers.getMeridiem(locale).map((v) => v.upper).join("|")}`);
	}
};
...
const parseFromFormat = (text, format, previousValue, locale, timezone) => {
	...
	let parsedDate = require_components_date_calendar_helpers.dayjsTimezone(require_components_date_calendar_helpers.isValidDate(previousValue) ? (0, dayjs.default)(previousValue) : (0, dayjs.default)(), timezone);
	sections.sort((a, b) => require_components_date_picker_constants.DATE_PICKER_FORMATS.findIndex((v) => v === a.format) - require_components_date_picker_constants.DATE_PICKER_FORMATS.findIndex((v) => v === b.format));
	for (const section of sections) {
		...
		switch (sectionFormat) {
			...
			case "MMM":
			case "MMMM":
				const monthIndex = require_components_date_calendar_helpers.getMonths(locale).findIndex((v) => sectionFormat === "MMM" ? v.short === value : v.long === value);
			...
			case "a":
			case "A":
				const meridiem = require_components_date_calendar_helpers.getMeridiem(locale).findIndex((v) => sectionFormat === "A" ? v.upper === value : v.lower === value);
	...
	return parsedDate.isValid() ? require_components_date_calendar_helpers.dateTypeToDateObject(parsedDate, timezone) : invalidDate;
};
const getDateformatSections = (inputValue, format, locale) => {
	...
	for (let i = 0; i < formatChunks.length; i++) {
		...
		const getOptions = () => {
			switch (formatChunk) {
				case "MMM": return require_components_date_calendar_helpers.getMonths(locale).map((v) => v.short);
				case "MMMM": return require_components_date_calendar_helpers.getMonths(locale).map((v) => v.long);
				case "a": return require_components_date_calendar_helpers.getMeridiem(locale).map((v) => v.lower);
				case "A": return require_components_date_calendar_helpers.getMeridiem(locale).map((v) => v.upper);
				default: return [];
			}
		};
		result.push({
...
const isDatePickerFormat = (format) => {
	return require_components_date_picker_constants.DATE_PICKER_FORMATS.includes(format);
};
const localeFormat = (value, format, locale, timezone) => {
	if (!require_components_date_calendar_helpers.isValidDate(value)) return value;
	const parsedValue = require_components_date_calendar_helpers.dateTypeToDateObject(value, timezone);
	const parsedDayjs = require_components_date_calendar_helpers.dayjsTimezone((0, dayjs.default)(parsedValue), timezone);
	switch (format) {
...
const getNumericFormatRange = (format, value, timezone) => {
	const maxDaysInMonthFromValue = require_components_date_calendar_helpers.dayjsTimezone((0, dayjs.default)(require_components_date_calendar_helpers.isValidDate(value) ? require_components_date_calendar_helpers.dateTypeToDateObject(value, timezone) : require_components_date_calendar_helpers.dateTypeToDateObject(/* @__PURE__ */ new Date(), timezone)), timezone).daysInMonth();
...
const getIncrementedSectionValue = (section, direction, value, timezone) => {
	...
	if (isNaN(parsed)) {
		if (section.format === "YY" || section.format === "YYYY") return require_components_date_calendar_helpers.dayjsTimezone((0, dayjs.default)(), timezone).format(section.format);
	...
	if (section.type === "text") {
		const foundOption = section.options.filter((v) => {
			if (/^a$/i.test(section.format)) {
				const [am, pm] = require_components_date_calendar_helpers.getMeridiem(locale).map((m) => section.format === "a" ? m.lower : m.upper);
...
//#endregion
exports.getBoundSectionValue = getBoundSectionValue;
exports.getClosetSection = getClosetSection;
exports.getDateformatSections = getDateformatSections;
exports.getIncrementedSectionValue = getIncrementedSectionValue;
exports.getNumericFormatRange = getNumericFormatRange;
exports.getRegexFormat = getRegexFormat;
exports.invalidDate = invalidDate;
exports.localeFormat = localeFormat;
exports.parseFromFormat = parseFromFormat;
exports.parseSections = parseSections;
exports.processCharacterInput = processCharacterInput;
