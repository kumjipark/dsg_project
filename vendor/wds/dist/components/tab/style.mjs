...
const tabListStyle = ({ isScrollableLeft, isScrollableRight, resize, horizontalPadding, size, xs, sm, md, lg, xl }) => (theme) => css`
    width: 100%;
    list-style: none;
    position: relative;
    padding: 0;
	...
	xl
        ...
        ${(Boolean(params?.resize) || Boolean(params?.size) || params?.horizontalPadding !== void 0) && css`
          ${tabPaddingStyle({
      ...
      `;
        ...
        ${isScrollableLeft || isScrollableRight ? css`
              ...
              &:not(:has([data-role='tab-list-icon-button']))
                ...
                mask-image: ${[isScrollableLeft && getGradientMaskImage("left", "48px", "mask"), isScrollableRight && getGradientMaskImage("right", "48px", "mask")].filter(Boolean).join(", ")};
              }
            ` : css`
              &:not(:has([data-role='tab-list-icon-button']))
                [data-radix-scroll-area-wrapper],
...
`;
const tabListWrapperStyle = (theme) => css`
  position: relative;
...
const motionDividerStyle = css`
  ...
  display: none;
`;
const tabListItemStyle = ({ disabled }) => (theme) => css`
  ...
  `;
const tabListItemInteractionStyle = css`
  position: absolute;
