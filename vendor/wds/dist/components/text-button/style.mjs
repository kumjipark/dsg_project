...
const textButtonStyle = ({ loading, xs, sm, md, lg, xl, ...props }) => (theme) => css`
    display: inline-flex;
    align-items: center;
...
const getColorTheme = ({ color, overrideColor }, theme) => {
	switch (color) {
        ...
        color: ${overrideColor ? getColorByToken(theme, overrideColor) : theme.semantic.primary.normal};
      ...
      `;
		case "assistive": return css`
        background-color: transparent;
        border: none;
        box-shadow: none;
        color: ${overrideColor ? getColorByToken(theme, overrideColor) : theme.semantic.label.alternative};
